...
def run_stage2_ar(config: dict):
    ...
    # ── XQuAD-ar val pairs (rank 0 only) ────────────────────
    val_pairs = None
    if is_main_process():
        sys.path.insert(0, os.path.join(config['root_dir'], 'arabic'))
        from data.xquad_loader_ar import create_xquad_ar_dataloaders
        _, _, val_pairs = create_xquad_ar_dataloaders(
    ...
    if is_main_process():
        log.info(f'Train (SQuAD Parallel AR): {len(train_loader)} batches/GPU | XQuAD-ar Val: {len(val_pairs) if val_pairs else 0} pairs')
    ...
    for epoch in range(start_epoch, config['max_epochs'] + 1):
        ...
        if is_main_process():
            ...
            # Margin annealing (same logic as VI)
            if config.get('anneal_margin'):
                if epoch < margin_warmup_epochs:
                    ...
                    if next_idx != current_margin_idx:
                        ...
                        log.info(f'  [Margin] Warmup step → margin={margin_schedule[current_margin_idx]:.1f}')
                    best_ar_f1 = ar_f1
                    margin_patience_count = 0
                else:
                    if ar_f1 > best_ar_f1 + 0.1:
                        best_ar_f1 = ar_f1
                        margin_patience_count = 0
                    else:
                        margin_patience_count += 1
                        if margin_patience_count >= 1 and current_margin_idx < len(margin_schedule) - 1:
                            current_margin_idx += 1
                            log.info(f'  [Margin] Stepping down → {margin_schedule[current_margin_idx]:.1f}')
                        margin_patience_count = 0
        ...
        # Broadcast break signal
        if world_size > 1:
            signal_tensor = torch.tensor([1 if should_break else 0, current_margin_idx], device=device)
